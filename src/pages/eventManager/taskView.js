...
const TaskView = () => {
  ...
  const getTaskDetail = async (taskId) => {
    await axios
      .get(`http://65.2.129.99/api/eventManager/tasks/task-dettail/`, {
  ...
  console.log(empIdList)
    ...
    const response = await axios
      .post(
        `http://65.2.129.99/api/eventManager/tasks/updateTask/`,updatedTaskData, {
          params: { taskId: taskId }, 
        }
      )
      .then((response) => {
  ...
  const getAssignedEmployees = async (taskId) => {
    const response = await axios
      .get(
        `http://65.2.129.99/api/eventManager/task/assigned-employees/${taskId}`
      )
      .then((response) => {
        const employees =
